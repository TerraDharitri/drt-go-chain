...
func TestGovernanceContract_ExecuteSendDCDT(t *testing.T) {
	t.Parallel()
	...
	callInput.DCDTTransfers = []*vmcommon.DCDTTransfer{{DCDTValue: big.NewInt(10), DCDTTokenName: []byte("tokenName"), DCDTTokenType: 0, DCDTTokenNonce: 0}}
	retCode := gsc.Execute(callInput)
	require.Equal(t, vmcommon.UserError, retCode)
	require.True(t, strings.Contains(eei.GetReturnMessage(), "cannot transfer DCDT to system SCs"))
}
...
func TestGovernanceContract_VoteAfterGovernanceFixesActivationWithOngoingListV1(t *testing.T) {
	...
	callInput := createVMInput(big.NewInt(0), "delegateVote", vm.DCDTSCAddress, vm.GovernanceSCAddress, delegateVoteArgs)
...
func TestGovernanceContract_DelegateVoteMoreErrors(t *testing.T) {
	...
	callInput.CallerAddr = vm.DCDTSCAddress
	retCode = gsc.Execute(callInput)
	require.Equal(t, vmcommon.UserError, retCode)
...
func TestGovernanceContract_DelegateVoteMultiple(t *testing.T) {
	...
	addStakeAndDelegationForAddress(gsc, vm.DCDTSCAddress)
	addStakeAndDelegationForAddress(gsc, vm.FirstDelegationSCAddress)
	callInput := createVMInput(big.NewInt(0), "delegateVote", vm.DCDTSCAddress, vm.GovernanceSCAddress, voteArgs)
	retCode := gsc.Execute(callInput)
	require.Equal(t, vmcommon.Ok, retCode)
