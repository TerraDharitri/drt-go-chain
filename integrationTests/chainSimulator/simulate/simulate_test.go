...
func TestCostScDeploy(t *testing.T) {
	...
	receiver := "drt1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq85hk5z"
...
func TestSimulateIntraShardTxWithGuardian(t *testing.T) {
	...
	dcdtTransferTx := generateTransaction(sender.Bytes, senderNonce, receiver.Bytes, big.NewInt(0), "DCDTTransfer@010101@01@01", 500_000)
	cost, err := cs.GetNodeHandler(0).GetFacadeHandler().ComputeTransactionGasLimit(dcdtTransferTx)
	require.NoError(t, err)
	...
	dcdtTransferTx.GuardianAddr = guardian.Bytes
	dcdtTransferTx.Options = 2
	dcdtTransferTx.Version = 2
	dcdtTransferTx.GuardianSignature = []byte("aaa")
	cost, err = cs.GetNodeHandler(0).GetFacadeHandler().ComputeTransactionGasLimit(dcdtTransferTx)
	require.NoError(t, err)
	...
	err = cs.SetStateMultiple([]*dtos.AddressState{
		{
			...
			Pairs: map[string]string{
				"4e554d4241546463647453484f572d633961633237": "12040002d820",
			},
		},
	})
	require.NoError(t, err)
